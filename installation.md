...
1. Choose your control node and control user.
    ...
    **If you want to use the installer, the control user must be a sudoer.**
    ...
    1. Compile Sira's control node and client binaries (Sira is small, so this should be quick):
        ...
        $ cargo install sira
        ```
    1. If you intend to install Sira manually, complete the steps below. If you intend to use the installer, skip to the next step.
        ...
        1. Generate the SSH key pair that Sira will use to log in to managed nodes (as the Sira user). This guide will assume that it the private key is  `~/.ssh/sira` and the public key is `~/.ssh/sira.pub`. Protecting all SSH keys used in this guide with strong and unique passwords is highly recommended but not required.
    ...
    1. For each managed node:
        ...
        1. To use Sira's installer, run the following as the control user:
        ...
        1. If you are instead performing a manual installation, please complete the following steps starting as the control user:
            1. Transfer the following files from the control user to the administrative user:
                ...
                ```
            1. Log into the administrative user and complete the following steps:
...
1. Transfer the manifest private key (e.g. `~/.ssh/manifest`) from the control user to the account where you will develop your manifests and tasks. Remove it from the control user.
1. Make note of the following invocation. When you wish to sign your manifest and task files, you will need to run the following (which you might want to put in a simple script on your development machine):
   ...
   ```
1. Configure ~/.ssh/config on control user to meet the following requirements.
    1. When connecting to managed nodes as the Sira user, use the login key (e.g. `~/.ssh/sira`).
    1. If convenient, set the default user when logging into managed nodes as the Sira user. If you don't do this here, you will need to write `<user>@<host>` in all of your manifest files.
